...
- **Mutual recursion**: every SQL backend computes the whole SCC
  through a single combined CTE with a `__tag` discriminator.
  Neither engine can express an SCC as CTEs referring to each
  other. Postgres additionally allows only one recursive term, so
