...
| [`refinement-annotations.md`](./refinement-annotations.md) | first slice implemented, static discharge designed | A head position typed by a proposition rather than a primitive type, so the witness is erasable and each rule owes a proof obligation |
